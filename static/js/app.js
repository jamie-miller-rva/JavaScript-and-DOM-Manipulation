...
//=========================================================================================================
// Direciton and Guidance: Use a state form in the HTML document and write JavaScript code that will 
// listen for events and search through the `shape` column to find rows that match user input.//
// ========================================================================================================

// Get a reference to the button on the page with the id property set to `filter-btn-shape`
var filterButtonShape = d3.select("#filter-btn-shape");

// Getting a reference to the input elements on the page with the id property set to 'state'      
var searchShape = d3.select('#shape-1');

// Use an event handler (the `on` function in d3) to attach a click event to the 'Filter by Shape' button
// Define with an anonymous function - inline
filterButtonShape.on("click", function(){
    // select and console.log the searchShape
    var searchShapeValue = searchShape.property("value");
    console.log("New Shape");
    console.log(searchShapeValue);

    // Create a custom filtering function for searchShape
    function filterTableShape(ufoReport) {
        // select the searchShape
        return ufoReport.shape == searchShapeValue;
    }
    // If the searchShape is not blank filter data using the filterTableShape function
    if (searchShapeValue != ''){
        var refinedUFOReportShape = tableData.filter(filterTableShape);
        // log resutls to the console
        console.log(refinedUFOReportShape);
    } else{
        // if the searchShape is blank no filter is applied
        refinedUFOReportShape = tableData;
    };
    ...
    // Loop through the refinedUFOreport
    refinedUFOReportShape.forEach((ufoReport) => {
        // Use d3 to append one table row `tr` for each ufo report object
        var row = tbody.append("tr");
        // Use `Object.entries` to console.log each ufo report value
        Object.entries(ufoReport).forEach(([key, value]) => {
